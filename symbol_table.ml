...
let rec lookup_identifier name = function
  | None -> raise (UnrecognizedIdentifier (Printf.sprintf "Unrecognized Identifier %s" name))
  | Some (table) -> 
    try
      StringMap.find name table.current_scope
    with Not_found ->
      lookup_identifier name table.parent
