...
let check (globals, functions, structs) =
  ...
  let check_dups variables =
    let rec dups = function
      ...
      | _ :: t -> dups t
    in
    dups (List.sort (fun (_, a) (_, b) -> compare a b) variables)
  in
  ...
  let symbol_table =
    List.fold_left
      (fun accum (type_, name) -> St.add_current_scope accum name type_)
      (St.add_scope (St.empty ()))
      globals
  in

  (* A map which maps function name to a function definition <function_name, function_definition> *)
  let function_decls : Ast.func_def StringMap.t =
    List.fold_left add_func StringMap.empty (functions @ built_in_functions)
  in

  (* A map containing all functions of a given struct <struct_name, <struct_function, function_definition>> *)
  let struct_functions : Ast.func_def StringMap.t StringMap.t =
    let add_methods map _struct =
      let methods =
        List.fold_left
          (fun map func -> StringMap.add func.func_name func map)
          StringMap.empty _struct.methods
      in
  ...
  (* A map containing all fields of a given struct and their types <struct_name, <field_name, field_type>>*)
  let struct_fields : Ast.ty StringMap.t StringMap.t =
    let add_fields map _struct =
      let fields =
        List.fold_left
          (fun map bind -> StringMap.add (snd bind) (fst bind) map)
          StringMap.empty _struct.fields
      in
  ...
  (* A function which searches for a particular value that belongs to a struct *)
  ...
  (* Checks to make sure that main function is declared *)
  let _ =
    try
      let main_func = find_func function_decls "main" in
      if main_func.return_type <> Void then
        raise (InvalidReturnType "main() should have void return type")
  ...
  let rec check_func symbol_table func =
            ...
            if List.length lst = 1 then fst (List.hd lst)
              ...
              | hd :: tl ->
                  ...
                  if ty = ty_rest then ty
                    ...
                    raise
                      (TypeMismatch
                         ( "Can't have sequence containing " ^ string_of_typ ty
                         ^ " and " ^ string_of_typ ty_rest ))
          in
          ...
          if t1 = t2 then
            let ty =
              ...
              | Sub | Mult | Div -> (
                  ...
                  | _ -> raise Bad_arithmetic )
              (* % *)
              | Mod -> ( match t1 with Int -> Int | _ -> raise Unimplemented )
              (* && || *)
              | And | Or -> (
                  match t1 with Bool -> Bool | _ -> raise Bad_logical )
              (* ==, != *)
              | Equal | Neq -> (
                  match t1 with
      ...
      | Call (name, arguments) as call ->
          let name, parameters, return_type =
            if func_exists function_decls name then
              ...
              let func =
                match St.lookup_identifier name table with
                | Function func -> func
                | _ -> raise Invalid
              in
          ...
          if List.length arguments <> len then
            raise
              (Failure
                 ( "expecting " ^ string_of_int len ^ " arguments in "
                 ^ string_of_expr call ))
          else
            let check_call ft e =
              let ty, e' = check_expr table e in
              let err =
                "illegal argument found " ^ string_of_typ ty ^ " expected "
            ...
            if name = "append" then
              ...
              | _ ->
                  raise
                    (IncorrectArgumentType
                       "Expected a list as first argument to append")
              ...
              | List rest ->
                  if
                    rest = fst (List.nth sargs 2)
                    && fst (List.nth sargs 1) = Int
                  then (return_type, SCall (name, sargs))
                  else raise Func_failed_typecheck
              | _ -> raise Func_failed_typecheck
              ...
              | List rest -> (
                  ...
                  | Function ([ input ], out) ->
                      if rest <> input then
                        raise
                          (IncorrectArgumentType
                             "Input to function should be same as list type")
                      else (List out, SCall (name, [ (ty, e1); (el_ty, e2) ]))
                  | _ ->
                      raise
                        (IncorrectArgumentType
                           "Input to map should be a list and function \
                            accepting list element") )
              | _ ->
                  raise
                    (IncorrectArgumentType
                       "Expected a list as first argument to append")
              ...
              | Type el_ty ->
                  ...
                  if arg2_ty = Int && arg3_ty = el_ty then
                    (List el_ty, SCall (name, sargs))
                  else
                    raise
                      (IncorrectArgumentType
                         "Incorrect arguments supplied to make()")
              | _ ->
                  raise
                    (IncorrectArgumentType
                       "Expected type of list elements as first argument")
            else
      ...
      | SeqAccess (var, inside_bracket) -> (
          ...
          | Int -> (
              ...
              | _ ->
                  raise
                    (IllegalAccess
                       (string_of_typ type_ ^ " is not a sequential type")) )
          | _ ->
              raise
                (IllegalAccess
                   ("Can't access sequential type with " ^ string_of_typ ty)) )
    ...
    and check_bool_expr table expr =
      ...
      match ty with Bool -> (ty, e') | _ -> raise Invalid
    (* Can come up with a better error message *)
    ...
    and check_stmt table = function
      | Return e ->
          ...
          if ty = func.return_type then (SReturn (ty, e'), table)
            ...
            raise
              (InvalidReturnType
                 (Printf.sprintf "Returned %s must be of type %s"
                    (string_of_expr e) (string_of_typ ty)))
      | If (expr, stmt) ->
          (SIf (check_bool_expr table expr, fst (check_stmt table stmt)), table)
      | Block lst ->
          let new_table = St.add_scope table in
      ...
      | AssignSeq (seq, index, var) ->
          ...
          if el_ty = vartype then
            (SAssignSeq ((ty, e'), (ind_ty, sindex), (vartype, svar)), table)
          else raise InvalidAssignment
      | Define (name, expr) ->
          let expr_ty, e' = check_expr table expr in
          let table = St.add_current_scope table name expr_ty in
      ...
      | IfElse (expr, stmt1, stmt2) ->
          ( SIfElse
              ( check_bool_expr table expr,
                fst (check_stmt table stmt1),
                fst (check_stmt table stmt2) ),
            table )
      | Iterate (x, e, stmt) -> (
          let ty, e' = check_expr table e in
          let get_siterate el_ty = function
            | Block lst ->
                (* flatten list *)
                let new_table =
                  St.add_current_scope (St.add_scope table) x el_ty
                in
                let sblock = fst (check_stmt_list new_table lst) in
      ...
      | Range (x, e1, e2, stmt) -> (
          ...
          let get_srange = function
            | Block lst ->
                (* flatten list *)
                let new_table =
                  St.add_current_scope (St.add_scope table) x Int
                in
                let sblock = fst (check_stmt_list new_table lst) in
    ...
    let sast, table =
      ...
      | Block lst ->
          check_stmt_list symbol_table lst (* flatten block to list *)
      | _ -> raise Invalid
    in
  ...
  let check_struct _struct =
    let symbol_table =
      List.fold_left
        (fun st (ty, name) -> St.add_current_scope st name ty)
        (St.add_scope symbol_table)
        _struct.fields
    in
    (* Create symbol table with struct vars then check_func for each of the methods *)
    {
      ...
      smethods =
        List.map (fun func -> check_func symbol_table func) _struct.methods;
    }
  in
  ( globals,
    List.map (fun func -> check_func symbol_table func) functions,
    List.map check_struct structs )
